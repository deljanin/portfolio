...
function SphereScene() {
	...
	useEffect(() => {
		...
		const particleSphere = new thre.Points(particleSphereGeometry, particlesSphereMaterial);
		...
		particlesGeometry.setAttribute('position', new thre.BufferAttribute(positionArr, 3));
		...
		const animate = () => {
			...
			particleSphere.rotation.x += 0.002 * (targetX - particleSphere.rotation.x);
	...
	const sphere = useRef();
	const isVisible = useElementOnScreen({ root: null, rootMargin: '0px', threshold: 0.3 }, sphere);

	return (
		<>
			<div className='sphere__container' id='sphere_section'>
				<div
					className={`sphere__grid ${isVisible ? 'sphere__visible' : 'sphere__notVisible'}`}
					ref={sphere}
				>
				...
				<canvas id='cvs' className={isVisible ? 'sphere__visible' : 'sphere__notVisible'} />
			</div>
		</>
	);
}
